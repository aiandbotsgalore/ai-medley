...
export function useSessionState() {
  const [sessionState, setSessionState] =
    useState<AutonomousSessionState | null>(null);
  ...
  const updatePhase = useCallback((phase: SessionPhase) => {
    ...
    setSessionState((prev) =>
      prev ? { ...prev, currentPhase: phase, updatedAt: Date.now() } : prev,
    );
  }, []);

  const updateMetrics = useCallback((metrics: CanonicalMetrics) => {
    setSessionState((prev) =>
      prev ? { ...prev, metrics, updatedAt: Date.now() } : prev,
    );
  }, []);
  ...
  const updateDesign = useCallback((d: MedleyDesignPayload | null) => {
    setDesign(d);
    setSessionState((prev) =>
      prev ? { ...prev, design: d, updatedAt: Date.now() } : prev,
    );
  }, []);

  const updateSemanticMemory = useCallback((memory: SemanticMemory) => {
    setSessionState((prev) =>
      prev ? { ...prev, semanticMemory: memory, updatedAt: Date.now() } : prev,
    );
  }, []);
  ...
  const incrementIteration = useCallback(() => {
    setSessionState((prev) =>
      prev
        ? { ...prev, iteration: prev.iteration + 1, updatedAt: Date.now() }
        : prev,
    );
  }, []);
  ...
  const markLastTool = useCallback((toolName: string) => {
    setSessionState((prev) =>
      prev ? { ...prev, lastTool: toolName, updatedAt: Date.now() } : prev,
    );
  }, []);
