...
import ExecutionContextPanel, {
  type ExecutionContextSummary,
} from "./components/ExecutionContextPanel";
import {
  buildSystemPrompt,
  getOpenRouterTools,
  getToolDeclarations,
...
import {
  validateCheckpoint,
  upgradeCheckpoint,
  enrichCheckpointPayload,
} from "./utils/checkpointManager";
import { postAutomaticCheckpoint } from "./utils/automaticCheckpoint";
...
import {
  ...
  supersededRunError,
} from "./utils/runGeneration";
import { runPreAnalysisPipeline } from "./utils/preAnalysisPipeline";
import { runStartMedleyPipeline } from "./utils/startMedleyPipeline";
import {
  classifyProviderFailure,
  getManualProviderFailureDecision,
  type ProviderRequestAudit,
} from "./engine/providerRequest";
import { parseManualToolCall } from "./engine/manualToolContracts";
...
type CheckpointData = {
  ...
  manualConfigBinding?: ManualConfigBinding;
  providerRequestAudits?: ProviderRequestAudit[];
};
...
function deriveExecutionContext(
  ...
  const reasoningSnippet = lastReasoning
    ? lastReasoning.replace(/\s+/g, " ").trim().slice(0, 160)
    : "";
  ...
  let ctx: ExecutionContextSummary = {
    ...
    rationale:
      "The agent follows a structured multi-phase loop (analyze → design → build → evaluate → refine → finish) to produce a musically coherent result.",
    impact:
      "Each step contributes data or decisions that improve the quality and seamlessness of the final medley.",
    nextStep:
      "The model will decide the next tool call or conclude the process.",
  };
  ...
  if (tool.includes("listen_to_audio") || p.toUpperCase().includes("ANALYZE")) {
    ctx = {
      phase: p || "ANALYZE",
      currentAction:
        "Performing deep analysis of a track’s waveform, energy, tempo, silence map, and timbral character",
      rationale:
        "No creative decisions can be trustworthy without objective sonic facts about every piece of source material.",
      impact:
        "This data powers the transition scoring matrix, recommended section candidates, and every later timing decision.",
      nextStep:
        "The agent will evaluate promising section pairs or load more cached analyses.",
    };
  } else if (tool.includes("evaluate_section_pair")) {
    ctx = {
      phase: p || "DESIGN",
      currentAction:
        "Scoring how musically compatible two specific sections are for a direct join",
      rationale:
        "Raw energy or key data is not enough — only direct compatibility testing reveals which pairs will actually feel seamless.",
      impact:
        "Only high-scoring pairs are allowed into the final locked design plan, dramatically reducing the chance of weak transitions.",
      nextStep:
        "Once enough pairs are scored the agent will call set_design_plan to lock the authoritative structure.",
    };
  } else if (tool.includes("set_design_plan")) {
    ctx = {
      phase: "DESIGN — STRUCTURE LOCKED",
      currentAction:
        "Committing to a final ordered sequence of sections and exact transition points",
      rationale:
        "After objective evaluation the agent now has enough confidence to freeze one concrete architecture for the rest of the run.",
      impact:
        "All future preview renders and the final single-pass render will use these exact timings and this exact ordering.",
      nextStep:
        "The agent will render real musical preview crossfades (apply_musical_transition) for the locked joins so you can audition them.",
    };
  } else if (tool.includes("apply_musical_transition")) {
    ctx = {
      phase: p || "BUILD",
      currentAction:
        "Rendering a high-quality preview crossfade for one locked transition using style-aware DSP and beat alignment",
      rationale:
        "The only reliable way to know whether a paper plan actually sounds good is to hear the real audio join.",
      impact:
        "The precise actual exit/entry timestamps returned (post beat-snap) become the authoritative values used in the final clean render.",
      nextStep:
        "After auditioning transitions the agent will either refine or call finalize_medley for the production master.",
    };
  } else if (
    tool.includes("analyze_medley_quality") ||
    tool.includes("report_progress")
  ) {
    ctx = {
      phase: p || "EVALUATE / REFINE",
      currentAction:
        "Measuring the current draft against hard quality metrics (loudness, smoothness, emotional arc, identity)",
      rationale:
        "Human ears are biased. Objective numbers tell the agent exactly which dimensions are still weak.",
      impact:
        "Low scores directly drive the next targeted refinement actions instead of random guessing.",
      nextStep:
        "The agent will either perform a focused refinement or decide the current version is ready for final rendering.",
    };
  } else if (tool.includes("finalize_medley")) {
    ctx = {
      phase: "FINISH — PRODUCTION RENDER",
      currentAction:
        "Executing the single deterministic final render from original sources only (sequential atrim + acrossfade chain + one master loudnorm + limiter)",
      rationale:
        "This is the authoritative, gap-free, timing-accurate production file. It never uses any pre-rendered preview assets.",
      impact:
        "This is the finished downloadable medley that represents the complete artistic vision.",
      nextStep:
        "Process complete. The final file is ready for playback and export.",
    };
  } else if (tool.includes("finish_medley")) {
    ctx.currentAction =
      "Persisting the completed medley and surfacing the final summary";
    ctx.nextStep = "You can now listen and download the result.";
  } else if (tool.includes("execute_shell")) {
    ctx = {
      phase: p,
      currentAction:
        "Executing a direct low-level command (usually FFmpeg or file I/O) in the session work directory",
      rationale:
        "Some operations still require direct shell access when no specialized tool exists for that exact step.",
      impact:
        "These commands are in service of the current phase (analysis, preview, or final export).",
      nextStep:
        "The command result is returned to the model so it can continue reasoning.",
    };
  }
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"workshop" | "history">(
    "workshop",
  );
  ...
  const [specialistRole, setSpecialistRole] = useState<SpecialistRole | null>(
    null,
  );
  ...
  const logDetailedError = useCallback(
    (context: string, error: any, extra?: any) => {
      ...
      let logMessage = `❌ [${context}] ${safeError.message}`;

      if (status) logMessage += ` | Status: ${status}`;
      if (safeError.rawBody)
        logMessage += `\n   Provider detail: ${typeof safeError.rawBody === "string" ? safeError.rawBody.substring(0, 800) : JSON.stringify(safeError.rawBody)}`;
      if (safeError.rawArguments)
        logMessage += `\n   Tool args: ${safeError.rawArguments}`;
      if (safeError.extra) {
        try {
          const extraStr =
            typeof safeError.extra === "string"
              ? safeError.extra
              : JSON.stringify(safeError.extra, null, 2);
          logMessage += `\n   Extra context: ${extraStr.substring(0, 1200)}`;
        } catch {}
      }

      // Also log full error to browser console for deeper inspection
      console.error(`[DETAILED ERROR - ${context}]`, safeError);
    ...
    const loadConfig = async () => {
      try {
        ...
        const serverConfig = await fetch("/api/config")
          .then((r) => r.json())
          .catch(() => ({}));
        const migration = migrateMedleyConfigWithNotices(
          storedConfig,
          serverConfig,
        );

        if (!cancelled) {
          setConfig(migration.config);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      CONFIG_STORAGE_KEY,
      JSON.stringify({ ...config, geminiApiKey: "", openrouterApiKey: "" }),
    );
  }, [config, configLoaded]);
  ...
  const handleFileDrop = (e: React.DragEvent) => {
    ...
    if (droppedFiles.length > 0) {
      uploadToLibrary(
        droppedFiles.filter((file) => file.type.startsWith("audio/")),
      );
    }
  };
  ...
  const shouldUploadAudioForAnalysis = (displayName: string) => {
    ...
    if (config.audioAnalysisMode === "ask") {
      return window.confirm(
        `Upload "${displayName}" to ${config.provider === "gemini" ? "Gemini" : "OpenRouter"} for deeper audio analysis? Local analysis will be used if you choose Cancel.`,
      );
    }
    return false;
  };
  ...
  const getLocalAnalysis = async (
    ...
    return data as {
      analysisText: string;
      analysis: unknown;
      medleyIntelligence?: unknown;
    };
  };

  const buildMedleyDesign = async (
    lib: LibraryFile[],
    signal?: AbortSignal,
  ) => {
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.error || "Medley intelligence design failed.");
  ...
  const runAutonomousLoop = async (
    ...
    const isActiveRequest = () =>
      activeRequestSequenceRef.current === requestSequence && !signal.aborted;
    const assertActiveRequest = () => {
      if (!isActiveRequest()) throw supersededRunError();
    };
    if (!hasProviderKey) {
      setErrorMessage(
    ...
    const sid =
      resumeState?.sessionId ?? Math.random().toString(36).substring(7);
    setSessionId(sid);
    providerRequestAuditsRef.current = [];
    modelFallback.setProviderAuditHandler((audit) => {
      if (!isActiveRequest()) return;
      const existingIndex = providerRequestAuditsRef.current.findIndex(
        (item) => item.requestId === audit.requestId,
      );
      if (existingIndex >= 0)
        providerRequestAuditsRef.current[existingIndex] = audit;
      ...
      if (audit.status === "failed") {
        addLog(
          `Provider attempt failed: ${audit.errorCategory ?? "unknown"} (${audit.model})`,
        );
      }
    });
    ...
    if (!resumeState) {
      ...
      addLog(
        `🔁 Resuming session ${sid} from iteration ${resumeState.iterations}`,
      );
    }
    ...
    connectSSE(sid, {
      ...
      onProgress: (data) => {
        if (isActiveRequest()) metricsManager.setRenderProgress(data);
      },
      onMetrics: (data) => {
    ...
    modelFallback.prepareForRun(resumeState?.currentModelIndex ?? 0);
    let session = modelFallback.createSessionForModel(
      modelFallback.getCurrentModel(),
      lib,
      design,
      sid,
      resumeState?.chatHistory,
    );
    ...
    (window as any).switchModelBack = () =>
      modelFallback.switchToPreviousModel(addLog);
    (window as any).resetModel = () =>
      modelFallback.resetToPrimaryModel(addLog);
    ...
    try {
      const sendWithRetry = async (msg: any): Promise<any> => {
        assertActiveRequest();
        try {
          const response = await session.send(msg, { signal });
          assertActiveRequest();
          return response;
        } catch (err: any) {
          assertActiveRequest();
          const providerError = classifyProviderFailure({ cause: err });
          const decision = getManualProviderFailureDecision(providerError);
          if (decision.action === "switch_model") {
            addLog(`⚠️ ${decision.userMessage}`);
            const switched = await switchToNextModel(
              `${providerError.category}${providerError.status ? ` (${providerError.status})` : ""}`,
            );
            if (switched) return sendWithRetry(msg);
            throw providerError;
          }
          if (
            decision.action === "reconfigure" ||
            decision.action === "reduce_payload" ||
      ...
      const saveCheckpoint = () => {
        const data: CheckpointData = {
          ...
          evaluationsPerFromSectionEntries: [
            ...evaluationsPerFromSection.entries(),
          ],
          manualConfigBinding: createManualConfigBinding(config),
          providerRequestAudits: [...providerRequestAuditsRef.current],
        };
        const enriched = enrichCheckpointPayload(
          data,
          sessionManager.sessionState?.semanticMemory,
      ...
      while (!loopFinished && iterations < MAX_ITERATIONS) {
        ...
        if (refinementPassCount >= MAX_LOOP_TURNS) {
          addLog(
            `⛔ Hard limit: maxLoopTurns (${MAX_LOOP_TURNS}) reached at iteration ${iterations}. Completed: ${evaluateCallCount} evaluations, ${refinementPassCount} turns. Phase at stop: ${currentPhaseRef.current}. Stopping.`,
          );
          break;
        }
        if (llmCallCount >= MAX_LLM_CALLS_PER_RUN) {
          addLog(
            `⛔ Hard limit: MAX_LLM_CALLS_PER_RUN (${MAX_LLM_CALLS_PER_RUN}) reached at iteration ${iterations}. Completed: ${evaluateCallCount} evaluations, ${refinementPassCount} refinements. Phase at stop: ${currentPhaseRef.current}. Stopping.`,
          );
          break;
        }
        ...
        metricsManager.setExecutionContext(
        ...
        if (!functionCalls || functionCalls.length === 0) {
          if (!result.text) {
            ...
            addLog(
              `⚠️ Model returned no tool calls and no text (streak: ${streak2}/${MAX_TOOL_FAILURE_STREAK})`,
            );
            if (
              ...
              const switched = await switchToNextModel(
                `Model returned empty responses ${streak2} times`,
              );
              if (!switched) break;
            }
            result = await sendWithRetry(
              "Please proceed with the next step. You must call a tool.",
            );
            llmCallCount++;
            cheapLLMCalls++;
            if (result.usage?.total_tokens) {
          ...
          addLog(
            `⚠️ Model returned text without a tool call (streak: ${streak3}/${MAX_TOOL_FAILURE_STREAK}). May not support function calling.`,
          );
          if (
            modelFallback.toolFailureStreakRef.current >=
            MAX_TOOL_FAILURE_STREAK
            ...
            const switched = await switchToNextModel(
              `Model returned text-only responses ${streak3} times — likely no tool/function-calling support`,
            );
            if (switched) {
              result = await sendWithRetry(
                "Please continue. You must use tool calls to make progress.",
              );
              llmCallCount++;
              cheapLLMCalls++;
              continue;
        ...
        if (functionCalls.length > 0) {
          addLog(
            `   → Model requested ${functionCalls.length} tool call(s): ${functionCalls.map((c: any) => c.name).join(", ")}`,
          );
        }
        ...
        for (const call of functionCalls) {
          ...
          try {
            args = parseManualToolCall(call.name, call.args, {
          ...
          const toolHash = `${call.name}:${JSON.stringify(args)}`;
          if (recentToolCallHashes.has(toolHash)) {
            addLog(
              `⚠️ Duplicate tool call detected: ${call.name} — returning early-exit result.`,
            );
            toolRes = {
              functionResponse: {
                name: call.name,
                ...
                response: {
                  status:
                    "This exact call was already made this session. Do not repeat it — advance to the next step.",
                },
              },
            };
            if (toolRes) toolResponses.push(redactSensitive(toolRes));
            continue;
          }
          recentToolCallHashes.add(toolHash);
          if (recentToolCallHashes.size > MAX_TOOL_HASH_MEMORY) {
            recentToolCallHashes.delete(
              recentToolCallHashes.values().next().value!,
            );
          }
          ...
          try {
            if (call.name === "execute_shell_command") {
              ...
              const isRenderAttempt =
                /ffmpeg|filter_complex|concat|medley_final|\.mp3|atempo|amerge|amix|aconcat|final\s*render|export|finalize/.test(
                  cmd,
                );
              if (isRenderAttempt) {
                addLog(
                  `  🚫 Blocked manual render shell command — use finalize_medley`,
                );
                toolRes = {
                  functionResponse: {
                    name: call.name,
                    ...
                    response: {
                      error:
                        "Manual shell rendering is disabled. The final medley MP3 must be produced by calling finalize_medley after set_design_plan, apply_musical_transition, and report_progress. Do not use execute_shell_command for audio rendering.",
                    },
                  },
                };
              } else {
                ...
                const res = await fetch("/api/exec", {
                  ...
                  body: JSON.stringify({
                    ...
                    sessionId: sid,
                    capabilityMode: config.manualCapabilityMode,
                  }),
                  signal,
                });
                const data = await res.json();
              ...
              const displayName =
                entry?.originalName || argBasename || "audio-output";
                ...
                if (!audioResp.ok) {
                  toolRes = {
                    functionResponse: {
                      ...
                      id: call.id,
                      response: {
                        error: `Audio file not found or unreadable: ${args.filePath}`,
                      },
                    },
                  };
                } else {
              ...
              addLog(
                `  🔬 Evaluating pair: ${args.fromSectionId} → ${args.toSectionId}`,
              );
              ...
              if (sectionPairCache.has(cacheKey)) {
                addLog(
                  `   ♻️ Cache hit for pair ${args.fromSectionId} → ${args.toSectionId}`,
                );
                toolRes = {
                  functionResponse: {
                    name: call.name,
                    id: call.id,
                    response: sectionPairCache.get(cacheKey),
                  },
                };
                continue;
              }
              ...
              if (currentCount >= 5) {
                addLog(
                  `   ⛔ Top-N=5 limit reached for ${fromKey}. Rejecting additional candidates locally.`,
                );
              ...
              if (evaluateCallCount >= MAX_EVALUATE_CALLS_PER_RUN) {
                addLog(
                  `⛔ Hard limit: MAX_EVALUATE_CALLS_PER_RUN (${MAX_EVALUATE_CALLS_PER_RUN}) reached at iteration ${iterations}. Completed: ${evaluateCallCount} evaluations, ${refinementPassCount} refinements. Phase at stop: ${currentPhaseRef.current}. Rejecting further evaluations.`,
                );
                toolRes = {
                  functionResponse: {
                    name: call.name,
                    id: call.id,
                    response: {
                      error: "Evaluation limit reached for this run",
                    },
                  },
                };
                continue;
              ...
              if (data.locallyRejected) {
                autoRejectedCount++;
                addLog(
                  `   🚫 Locally rejected (score ${data.localHeuristicScore?.toFixed(2) ?? "low"}) — no LLM cost.`,
                );
                ...
                if (score >= LOCAL_AUTO_ACCEPT_THRESHOLD) {
                  autoAcceptedCount++;
                  addLog(
                    `   ✅ High local confidence (${score.toFixed(2)}) — auto-accepted locally (ambiguous-only routing).`,
                  );
                } else if (score <= LOCAL_REJECTION_THRESHOLD) {
                  autoRejectedCount++;
                  addLog(
                    `   🚫 Low local confidence (${score.toFixed(2)}) — auto-rejected locally.`,
                  );
                } else {
                  addLog(
                    `   ⚖️ Ambiguous local score (${score.toFixed(2)}) — proceeding with full evaluation.`,
                  );
                }
              }
              ...
              addLog(
                `   Locking design plan: ${(args.transitions || []).length} transitions`,
              );
              const res = await fetch("/api/session/design-plan", {
                ...
                body: JSON.stringify({
                  ...
                  plan: { transitions: args.transitions },
                }),
                signal,
              });
              const data = await res.json();
              ...
              if (data.success !== false) {
                ...
                sessionManager.updateSemanticMemory(
                  updateSemanticMemoryOnToolResult(
                    sessionManager.sessionState?.semanticMemory ?? emptyMemory,
                    call.name,
                    args,
                    data,
                  ),
              ...
              addLog(
                `   Applying ${args.style} transition: ${args.fromSectionId} → ${args.toSectionId}`,
              );
              ...
              const res = await fetch(
                `/api/file-read?filePath=${encodeURIComponent(args.filePath)}&sessionId=${encodeURIComponent(sid)}`,
                { signal },
              );
              toolRes = {
                functionResponse: {
              ...
              addLog(
                `  📈 Scores: Arc=${normalizedMetrics.emotionalArc}% Trans=${normalizedMetrics.transitionSmoothness}% Identity=${normalizedMetrics.performerIdentity}% Overall=${normalizedMetrics.overallScore}%${phaseLog}`,
              );

              // === Early Termination — log only; let the agent naturally call finalize_medley next ===
              if (
                metricsManager.shouldTerminateEarly(
                  normalizedMetrics.overallScore,
                )
              ) {
                addLog(
                  `   ✅ Early termination: overallScore ${normalizedMetrics.overallScore} >= ${EARLY_TERMINATION_SCORE}. Stopping refinement.`,
                );
              }

              // Stall detection — embed guidance in function response (same turn, no protocol violation)
              const isStalled =
                metricsManager.isConverged(normalizedMetrics) &&
                iterations > 10;
              if (isStalled) {
                addLog(
                  `⚠️ Stalled progress detected at iteration ${iterations}.`,
                );
              }
              ...
              addLog(
                `  ⚠️ finish_medley called — redirecting to finalize_medley (no render was performed)`,
              );
              toolRes = {
                functionResponse: {
                  name: call.name,
                  ...
                  response: {
                    error:
                      "finish_medley does not produce an MP3. You must call finalize_medley with useCleanRender: true to render the final output audio file.",
                  },
                },
              };
              ...
              addLog(
                `  ✅ Reviewed candidate promoted byte-for-byte: ${data.outputPath}`,
              );
            ...
            if (
              ...
              const switched = await switchToNextModel(
                `Repeated failures calling tool "${call.name}" (${streak4} times)`,
              );
              if (switched) {
      ...
      if (iterations >= MAX_ITERATIONS && !loopFinished) {
        ...
        setStatus("error");
        setErrorMessage(
          "Max iterations reached without completing the medley.",
        );
      }

      // Phase 1 + Phase 2 stats at end of run
      addLog(
        `   [Phase 1/2 Final Stats] evaluateCalls=${evaluateCallCount}, llmCalls=${llmCallCount} (expensive=${expensiveLLMCalls}, cheap=${cheapLLMCalls}), loopTurns=${refinementPassCount}, cachedPairs=${sectionPairCache.size}, autoAccepted=${autoAcceptedCount}, autoRejected=${autoRejectedCount}`,
      );
      addLog(
        `   Note: Auto-accept threshold = ${LOCAL_AUTO_ACCEPT_THRESHOLD}, auto-reject threshold = ${LOCAL_REJECTION_THRESHOLD}. Recalibrate when using more diverse libraries.`,
      );
    } catch (e: any) {
      if (e.name === "AbortError" || !isActiveRequest()) {
        if (isActiveRequest()) {
          setStatus("idle");
  ...
  const runAutomaticWorkflow = async (
    ...
    const sid =
      resume?.sessionId ?? Math.random().toString(36).substring(2, 10);
    const requestSequence = ++activeRequestSequenceRef.current;
    providerRequestAuditsRef.current = [];
    (window as any).__providerRequestAudits = providerRequestAuditsRef.current;
    setSessionId(sid);
    ...
    try {
      ...
      const authorityData = await authorityResponse.json().catch(() => ({}));
      if (!authorityResponse.ok || !authorityData.design) {
        throw new Error(
          authorityData.error || "Could not establish server design authority",
        );
      }
      const authoritativeDesign = authorityData.design as MedleyDesignPayload;
      sessionManager.updateDesign(authoritativeDesign);
      const result = await runAutomaticSpecialistWorkflow({
        ...
        onStage: (stage, role, model) => {
          ...
          addLog(
            `${stageLabels[stage]}${role ? ` · ${role}` : ""}${model ? ` · ${model}` : ""}`,
          );
        },
      ...
      if (error?.name === "AbortError" || signal.aborted) {
        if (isActiveRequest()) setStatus("idle");
        return;
      }
      if (!isActiveRequest()) return;
      logDetailedError("Automatic Specialist Workflow", error, {
        sessionId: sid,
      });
      setStatus("error");
  ...
  const resumeFromCheckpoint = async (rawCheckpoint: StoredCheckpoint) => {
    ...
    if ((rawCheckpoint as AutomaticWorkflowCheckpoint).schemaVersion === 3) {
      const parsedCheckpoint =
        AutomaticWorkflowCheckpointSchema.safeParse(rawCheckpoint);
      if (!parsedCheckpoint.success) {
        setStatus("error");
        setErrorMessage(
          "This automatic checkpoint is invalid and cannot be resumed.",
        );
        return;
      }
      ...
      if (!compatibilityResponse.ok || compatibility.compatible !== true) {
        setStatus("error");
        setErrorMessage(
          `This checkpoint cannot be resumed safely (${compatibility.reason || "compatibility unknown"}). Start a new run or explicitly discard the checkpoint.`,
        );
        return;
      }
      cancelRunGeneration(runGenerationRef, abortRef.current);
      activeRequestSequenceRef.current++;
      const generation = beginRunGeneration(runGenerationRef);
      const controller = generation.controller;
      const isActiveRun = () =>
        abortRef.current === controller &&
        isRunGenerationActive(runGenerationRef, generation);
      abortRef.current = controller;
      setStatus("running");
      ...
      try {
        ...
        if (error?.name === "AbortError" || !isActiveRun()) {
          setStatus("idle");
          setRunStartedAt(null);
          return;
        }
        setStatus("error");
        setErrorMessage(
          error?.message || "Could not rebuild medley intelligence for resume",
        );
      }
      return;
    }
    ...
    if (!validateCheckpoint(legacyCheckpoint)) {
      addLog(
        "❌ Checkpoint failed validation — data may be corrupt. Discarding.",
      );
      setStatus("idle");
      return;
    }
    const checkpoint = upgradeCheckpoint(legacyCheckpoint);
    const configCompatibility = classifyManualCheckpointConfiguration(
      checkpoint.manualConfigBinding,
      config,
    );
    if (!configCompatibility.compatible) {
      setStatus("error");
      setErrorMessage(
        configCompatibility.reason === "legacy-unbound"
          ? "This legacy manual checkpoint is not bound to a provider/model configuration. Start a new run or explicitly discard it."
          : "The saved manual checkpoint uses a different provider/model configuration. Restore that configuration, start a new run, or explicitly discard it.",
      );
      return;
    }
    cancelRunGeneration(runGenerationRef, abortRef.current);
  ...
  const preAnalyzeLibrary = async (
    lib: LibraryFile[],
    signal: AbortSignal,
    isActiveRun: () => boolean = () => !signal.aborted,
    analysisGeneration = 0,
  ): Promise<void> => {
    await runPreAnalysisPipeline({
      entries: lib,
      signal,
      isActiveRun,
      concurrency: 3,
      // The server owns the content/version cache decision. A field-presence
      // check here can reuse stale facts after source or analyzer changes.
      isAnalyzed: () => false,
      onStart: (total) => {
        metricsManager.setPreAnalysisProgress({ current: 0, total });
        setCurrentPhase("ANALYZE — Pre-analyzing Library");
        addLog(`🔬 Pre-analyzing ${total} track(s) before session starts...`);
      },
      analyzeEntry: async (entry) => {
        const isStopped = () => signal.aborted || !isActiveRun();
        if (isStopped()) return;
        ...
        try {
          ...
          if (
            config.modelMode === "manual" &&
            shouldUploadAudioForAnalysis(entry.originalName)
          ) {
            if (isStopped()) return;
            addLog(`  ☁️ Cloud audio upload allowed for ${entry.originalName}`);
            const audioRes = await fetch(`/api/audio-raw/${entry.id}`, {
              signal,
            });
            if (isStopped()) return;
            ...
            const cloudAnalysisText = await analyzeAudioWithProvider({
              ...
              prompt:
                "Analyze this audio track and provide BPM if discernible, musical key, mood or genre, energy level from 1 to 10, and a concise 2 to 3 sentence structural summary.",
              signal,
            });
            if (isStopped()) return;
          ...
          if (analysisText) {
            ...
            if (
              config.modelMode === "manual" &&
              config.audioAnalysisMode !== "local"
            ) {
          ...
          if (e.name === "AbortError" || isStopped()) return;
          addLog(
      ...
      onProgress: (current, total) => {
        metricsManager.setPreAnalysisProgress({ current, total });
      },
      refreshLibrary: fetchLibrary,
      onClearProgress: () => metricsManager.setPreAnalysisProgress(null),
      onComplete: () => {
        setCurrentPhase("");
  ...
  const startMedley = async () => {
    ...
    await runStartMedleyPipeline<LibraryFile, MedleyDesignPayload>({
      ...
      buildDesign: async (freshLib, signal) => {
        ...
        addLog(
          `✅ Medley Intelligence ready: ${design.recommendedStrategies.length} strategies, ${design.transitionMatrixSummary.length} transition scores.`,
        );
        return design;
      },
      runAutomaticWorkflow,
      runManualWorkflow: (freshLib, signal, design) => {
        runAutonomousLoop(freshLib, signal, design);
      },
    });
  };
    ...
    <div className="min-h-[100dvh] md:h-[100dvh] bg-[#060606] text-[#E0E0E0] font-sans flex flex-col overflow-y-auto overflow-x-hidden md:overflow-hidden selection:bg-[#00F0FF]/30">
      ...
      <main className="flex-1 min-h-0 flex flex-col md:flex-row overflow-visible md:overflow-hidden">
        ...
        <section className="flex-1 min-w-0 min-h-[70vh] md:min-h-0 flex flex-col bg-[#030303] overflow-hidden shadow-[inset_1px_0_0_rgba(255,255,255,0.03)]">
            ...
            {(["workshop", "history"] as const).map((tab) => (
              <button
                ...
                onKeyDown={(event) => {
                  ...
                  const next = getAdjacentTab(activeTab, event.key as "ArrowLeft" | "ArrowRight" | "Home" | "End");
                  setActiveTab(next);
                  requestAnimationFrame(() => document.getElementById(`workspace-tab-${next}`)?.focus());
                }}
                className={`min-h-11 px-3 py-1 text-[11px] font-mono uppercase tracking-widest rounded transition-all ${
                  activeTab === tab
                    ? "text-[#00F0FF] bg-[#00F0FF]/10 border border-[#00F0FF]/20"
                    : "text-[#AAA] hover:text-white border border-transparent"
                }`}
              >
                {tab}
              </button>
          ...
          {status === "running" && (
            <div role="status" aria-live="polite" className="shrink-0 border-b border-[#1A1A1A] bg-[#0A0A0A] px-3 md:px-5 py-2.5 flex flex-col md:flex-row md:items-center justify-between gap-2 text-[11px] font-mono animate-fade-in">
              <div className="flex items-center gap-3 min-w-0">
                <span className="uppercase tracking-[1.5px] text-[#00F0FF] font-bold shrink-0">
                  CURRENT PHASE
                </span>
                <span className="text-white font-medium truncate">
                  {metricsManager.preAnalysisProgress
                    ...
                    : metricsManager.metrics?.phase ||
                      sessionManager.currentPhase ||
                      (metricsManager.iteration
                        ? "BUILD — Constructing Medley"
                        : "Initializing...")}
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 md:gap-3">
                ...
                {metricsManager.preAnalysisProgress && (
                  <div className="flex items-center gap-3 ml-4 min-w-[220px]">
                    <div
                      role="progressbar"
                      aria-label="Pre-analysis progress"
                      aria-valuemin={0}
                      aria-valuemax={metricsManager.preAnalysisProgress.total}
                      aria-valuenow={metricsManager.preAnalysisProgress.current}
                      className="flex-1 h-1.5 bg-[#1A1A1A] rounded-full overflow-hidden"
                    >
                      <div
                        className="h-full bg-gradient-to-r from-[#00F0FF] to-[#0080FF] transition-all duration-200"
                        style={{
                          width: `${(metricsManager.preAnalysisProgress.current / metricsManager.preAnalysisProgress.total) * 100}%`,
                        }}
                      />
                    </div>
                    <div className="text-[#888] tabular-nums w-12 text-right">
                      {Math.round(
                        (metricsManager.preAnalysisProgress.current /
                          metricsManager.preAnalysisProgress.total) *
                          100,
                      )}
                      %
                    </div>
                ...
                {/* Render progress bar */}
                {metricsManager.renderProgress &&
                  metricsManager.renderProgress.percent < 100 && (
                    <div className="flex items-center gap-3 ml-4 min-w-[260px] animate-pulse">
                      ...
                      <div
                        ...
                        aria-valuemin={0}
                        aria-valuemax={100}
                        aria-valuenow={metricsManager.renderProgress.percent}
                        className="flex-1 h-1.5 bg-[#1A1A1A] rounded-full overflow-hidden relative"
                      >
                        <div
                          className="h-full bg-gradient-to-r from-[#FF00F0] to-[#00F0FF] transition-all duration-200"
                          style={{
                            width: `${metricsManager.renderProgress.percent}%`,
                          }}
                        />
                      </div>
                      ...
                      {metricsManager.renderProgress
                        .remainingSecondsEstimate !== undefined &&
                        metricsManager.renderProgress
                          .remainingSecondsEstimate !== null && (
                  ...
                  !metricsManager.renderProgress && (
                    <div className="text-[#666] shrink-0">
                      ...
                      <span className="text-white font-medium">
                        {metricsManager.iteration.current}
                      </span>{" "}
                      / {metricsManager.iteration.max}
                    </div>
                  )}
          ...
          {activeTab === "history" ? (
            ...
            <div className="flex-1 p-4 md:p-8 flex flex-col items-center justify-center">
              ...
              >
                <Upload aria-hidden="true" className="w-12 h-12 text-[#777] group-hover:text-[#00F0FF] transition-colors mx-auto mb-6" />
                <h2 className="text-[15px] font-bold uppercase tracking-wider text-white mb-2">
                  Drop Audio Files Here
                </h2>
                <p id="audio-upload-help" className="text-[#AAA] text-[12px]">
                  MP3, WAV, FLAC, AAC, OGG • Click to browse
                </p>
                <input
              ...
              {isUploading && uploadProgress && (
                <div className="mt-8 border border-[#00F0FF]/20 bg-[#00F0FF]/[0.03] p-4 rounded-xl w-full max-w-lg">
                  ...
                  <div
                    ...
                    className="w-full bg-[#111] h-1.5 rounded-full overflow-hidden"
                  >
                    <div
                      className="bg-gradient-to-r from-[#00F0FF] to-[#0080FF] h-full rounded-full transition-all duration-300"
                      style={{
                        width: `${(uploadProgress.current / uploadProgress.total) * 100}%`,
                      }}
                    />
                  </div>
                  <div className="mt-1.5 text-[10px] font-mono text-[#444] text-right">
              ...
              {checkpoints.length > 0 && (
                <div className="mt-8 w-full max-w-lg border border-[#00F0FF]/20 bg-[#00F0FF]/[0.03] rounded-xl p-4">
                  <div className="text-[10px] font-mono uppercase tracking-widest text-[#00F0FF] font-bold mb-3">
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="text-[11px] text-white font-mono truncate">
                          {"workflowMode" in cp
                            ? cp.activeModel || "Automatic Specialist Team"
                            : cp.model}
                        </div>
              ...
              <button
                ...
                className="mt-10 min-h-11 px-10 py-3.5 bg-gradient-to-r from-[#00F0FF] to-[#0080FF] text-black text-[12px] font-bold uppercase rounded-xl hover:shadow-xl hover:shadow-[#00F0FF]/20 transition-all duration-300 disabled:opacity-100 disabled:from-[#555] disabled:to-[#555] disabled:text-white disabled:cursor-not-allowed disabled:shadow-none flex items-center gap-2 group"
              >
                <Play className="w-4 h-4" />
                Initialize Architecture
              </button>
              {library.length < 2 && library.length > 0 && (
                <p className="mt-3 text-[10px] text-[#AAA] font-mono">
                  Need at least 2 tracks to build a medley
                </p>
              )}
              {library.length >= 2 && !hasProviderKey && (
                <p className="mt-3 text-[10px] text-[#AAA] font-mono">
                  Open Configuration and add a{" "}
                  {config.provider === "gemini" ? "Gemini" : "OpenRouter"} API
                  key
                </p>
              )}
            </div>
            ...
            <LogPanel
              status={status}
              logs={logs}
              iteration={metricsManager.iteration}
      ...
      <footer className="min-h-16 border-t border-[#1A1A1A] bg-[#0A0A0A]/95 backdrop-blur flex items-center px-3 md:px-6 gap-3 md:gap-6 shrink-0">
        {status === "completed" && sessionId ? (
          ...
          <>
            ...
            <div className="hidden sm:block text-[10px] font-mono text-[#AAA]">
              44.1kHz • Stereo • 320kbps
            </div>
          </>
        )}
      </footer>
      ...
      <style
        ...
        * { font-family: 'Segoe UI', ui-sans-serif, system-ui, -apple-system, sans-serif; }
        code, .font-mono { font-family: 'Cascadia Mono', Consolas, ui-monospace, monospace !important; }
