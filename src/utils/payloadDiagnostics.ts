...
export function analyzePayload(payload: any): PayloadMetrics {
  ...
  const breakdown = {
    ...
    rankedCandidates: JSON.stringify(payload.rankedHookCandidates || payload.rankedCandidates || []).length,
    warnings: JSON.stringify(payload.warnings || []).length,
    other: 0,
  };

  breakdown.other = Math.max(0, totalChars -
...
export function logPayloadMetrics(metrics: PayloadMetrics, label = 'Gemini Payload') {
  console.log(`\n=== ${label} ===`);
  ...
  console.log(`Tracks: ${metrics.trackCount} | Sections: ${metrics.sectionCount}`);
  console.log('Breakdown (chars):');
  console.log(`  localFacts:        ${metrics.breakdown.localFacts.toLocaleString()}`);
  console.log(`  heuristicGuesses:  ${metrics.breakdown.heuristicGuesses.toLocaleString()}`);
  console.log(`  sectionScores:     ${metrics.breakdown.sectionScores.toLocaleString()}`);
  console.log(`  rankedCandidates:  ${metrics.breakdown.rankedCandidates.toLocaleString()}`);
  console.log(`  warnings:          ${metrics.breakdown.warnings.toLocaleString()}`);
  console.log(`  other:             ${metrics.breakdown.other.toLocaleString()}`);
  console.log('========================\n');
}
