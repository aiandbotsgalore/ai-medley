...
const trackB = buildTrackIntelligence({
  ...
  analysis: {
    ...
    candidateSections: [
      { start: 70, end: 112, energy: 92 },
      { start: 150, end: 188, energy: 86 },
    ],
  },
});
...
assert.equal(
  trackA.localFacts.every((fact) => fact.kind === "local_fact"),
  true,
);
assert.equal(
  trackA.heuristicGuesses.every((guess) => guess.kind === "heuristic_guess"),
  true,
);
assert.ok(
  trackB.rankedHookCandidates[0].scores.hookStrength >
    trackA.rankedHookCandidates[0].scores.hookStrength,
);
...
assert.ok(
  badPlan.errors.some((error) => error.includes("unsupported section id")),
);
    ...
    finalMedleyPlan: [
      {
        ...
        claim:
          "Emotional arc proxy based on local energy only; lyrics unavailable.",
      },
    ],
  },
  design,
