...
export const functionTool = (name: string, description: string, parameters: unknown) => ({
...
export function buildContextStageData(
  ...
  return {
    ...
    tracks: design.tracks.map(track => ({
      ...
      sections: (sectionsByTrack.get(track.trackId) ?? []).slice(0, 8).map(section => ({
...
export function buildQualityReviewStageData(input: {
  ...
  return {
    ...
    localQuality: {
      ...
      warnings: uniqueLimited(Array.isArray(quality.warnings) ? quality.warnings : [], 12),
    },
    executionReport: {
      ...
      attemptedTransitions: input.executionReport.attemptedTransitions.map(item => ({
      ...
      technicalWarnings: uniqueLimited(input.executionReport.technicalWarnings, 12),
      unresolvedFailures: uniqueLimited(input.executionReport.unresolvedFailures, 12),
    },
    correctionCount: input.correctionCount,
  };
}
