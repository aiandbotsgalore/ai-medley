...
const tempDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "ai-medley-local-analysis-"),
);
...
try {
  ...
  assert.equal(
    result.analysis.localAnalysisV2.schemaVersion,
  ...
  assert.equal(
    result.analysis.localAnalysisV2.baseAnalyzerVersion,
    LOCAL_ANALYZER_VERSION,
  );
  assert.ok(
    result.analysis.duration >= 11.9 && result.analysis.duration <= 12.1,
  );
  ...
  assert.ok(
    result.medleyIntelligence.localFacts.some(
      (fact) => fact.name === "spectral_centroid",
    ),
  );
  assert.ok(
    result.medleyIntelligence.localFacts.some(
      (fact) => fact.name === "beat_grid",
    ),
  );
  assert.ok(
    result.medleyIntelligence.sectionScores.every(
      (score) => typeof score.scores.beatAlignment === "number",
    ),
  );
  assert.ok(JSON.stringify(result.analysis.localAnalysisV2).length < 500_000);
  const sourceHash = hashAnalysisSource(wavPath);
  assertValidLocalAnalysis(result.analysis, sourceHash);
  assert.equal(isReusableLocalAnalysis(result.analysis, sourceHash), true);

  const invalidPath = path.join(tempDir, "invalid.mp3");
  ...
  await assert.rejects(
    ...
    (error: any) => error?.name === "AnalysisProcessError",
  );

  const controller = new AbortController();
  const cancelled = analyzeLocalAudioFile({
    filePath: wavPath,
    workDir: tempDir,
