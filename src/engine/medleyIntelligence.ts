...
export interface MedleyOrderStrategy {
  strategyId:
    | "smoothest_order"
    | "best_emotional_arc_proxy"
    | "highest_intensity"
    | "surprise_contrast"
    | "live_showcase"
    | "shortest_strong_medley";
  title: string;
  score: number;
  confidence: number;
...
function scoreTempoCompatibility(a: number | null, b: number | null) {
  if (!a || !b)
    return {
      score: 0.45,
      confidence: 0.25,
      warning: "tempo_missing_or_low_confidence",
    };
  const diff = Math.abs(a - b);
  const halfDoubleDiff = Math.min(
    Math.abs(a * 2 - b),
    Math.abs(a / 2 - b),
    Math.abs(b * 2 - a),
    Math.abs(b / 2 - a),
...
function parseKeyEstimate(value?: string | null) {
  ...
  return {
    root: chroma[match[1]],
    scale: match[2].toLowerCase() as "major" | "minor",
  };
}
...
function scoreHarmonicCompatibility(a?: TrackProfile, b?: TrackProfile) {
  ...
  if (!first || !second || confidence < 0.22) {
    return {
      score: 0.5,
      confidence,
      warning: "key_estimate_missing_or_low_confidence",
    };
  }
  const semitone = Math.min(
    Math.abs(first.root - second.root),
    12 - Math.abs(first.root - second.root),
  );
...
function sectionEnergy(
  curve: number[],
  section: Pick<SectionCandidate, "startSec" | "endSec">,
  duration: number,
) {
  if (!curve.length || duration <= 0) return 0;
  const startIndex = Math.max(
    0,
    Math.floor((section.startSec / duration) * curve.length),
  );
  const endIndex = Math.max(
    startIndex + 1,
    Math.ceil((section.endSec / duration) * curve.length),
  );
  return normalizeEnergy(average(curve.slice(startIndex, endIndex)));
}

function sectionPeak(
  curve: number[],
  section: Pick<SectionCandidate, "startSec" | "endSec">,
  duration: number,
) {
...
function dedupeAndMergeSections(
  sections: SectionCandidate[],
  minUsefulSec: number,
) {
  const sorted = [...sections].sort(
    (a, b) => a.startSec - b.startSec || b.durationSec - a.durationSec,
  );
  ...
  for (const section of sorted) {
    ...
    const tiny =
      section.durationSec < minUsefulSec &&
      !section.labels.includes("transition_safe_zone");
    ...
    if (previous && (overlaps || tiny)) {
      ...
      previous.labels = Array.from(
        new Set([...previous.labels, ...section.labels]),
      );
      previous.confidence = round(
        Math.max(previous.confidence, section.confidence),
      );
      previous.factsUsed = Array.from(
        new Set([...previous.factsUsed, ...section.factsUsed]),
      );
      previous.warnings = Array.from(
        new Set([...previous.warnings, ...section.warnings]),
      );
    } else {
      merged.push({ ...section });
    }
...
export function buildTrackIntelligence(input: {
  ...
  const curve = (analysis.energyCurve || []).filter((value) =>
  ...
  const dynamicRangeProxy = clamp01(
    (analysis.maxVolumeDb ?? -6) - (analysis.meanVolumeDb ?? -24) > 0
      ? ((analysis.maxVolumeDb ?? -6) - (analysis.meanVolumeDb ?? -24)) / 30
      : energySpread,
  );
  const sonicDensityProxy =
    v2?.spectral?.density?.average ??
    clamp01(avgEnergy * 0.6 + energySpread * 0.4);
  const brightnessProxy =
    v2?.spectral?.brightness?.average ??
    clamp01(0.25 + peakEnergy * 0.35 + energySpread * 0.25);
  ...
  const onsetDensityProxy = v2?.onsets?.densityPerMinute
    ...
    : clamp01(
        (analysis.candidateSections?.length || 0) * 0.12 +
          (duration
            ? (energyDeltaEvents / Math.max(1, duration / 60)) * 0.08
            : 0),
      );
  ...
  if (!analysis.estimatedBpm)
    warnings.push("tempo_estimate_missing_or_uncertain");
  if (v2?.tonal?.keyEstimate)
    warnings.push("key_is_estimated_locally_not_confirmed");
  ...
  const tempoConfidence =
    v2?.beatGrid?.confidence ?? (analysis.estimatedBpm ? 0.62 : 0.15);
  const profile: TrackProfile = {
    ...
    spectralCentroidHz: v2?.spectral?.centroidHz?.average
      ? round(v2.spectral.centroidHz.average, 1)
      : undefined,
    spectralRolloffHz: v2?.spectral?.rolloffHz?.average
      ? round(v2.spectral.rolloffHz.average, 1)
      : undefined,
    onsetDensityPerMinute: v2?.onsets?.densityPerMinute
      ? round(v2.onsets.densityPerMinute, 1)
      : undefined,
    ...
    confidence: round(
      clamp01(
        (duration ? 0.2 : 0) +
          (curve.length ? 0.22 : 0) +
          tempoConfidence * 0.2 +
          (analysis.silence?.length || 0 ? 0.1 : 0) +
          (v2?.advancedAnalysisAvailable ? 0.2 : 0) +
          (v2?.tonal?.confidence ? v2.tonal.confidence * 0.08 : 0),
      ),
    ),
  };
  ...
  const localFacts: LocalFact[] = [
    {
      ...
      trackId,
      value: duration || null,
      confidence: duration ? 0.95 : 0,
      source: "local_audio_analysis",
      warnings: duration ? [] : ["duration_unknown"],
    },
    {
      kind: "local_fact",
      name: "rms_energy",
      trackId,
      value: round(avgEnergy),
      confidence: curve.length ? 0.75 : 0,
      source: "local_audio_analysis",
      warnings: [],
    },
    {
      ...
      name: "peak_level",
      trackId,
      value: analysis.maxVolumeDb ?? null,
      confidence:
        analysis.maxVolumeDb === null || analysis.maxVolumeDb === undefined
          ? 0
          : 0.8,
      source: "local_audio_analysis",
      warnings: [],
    },
    {
      ...
      name: "tempo_estimate",
      trackId,
      value: analysis.estimatedBpm || null,
      confidence: tempoConfidence,
      source: "local_audio_analysis",
      warnings: analysis.estimatedBpm
        ? []
        : ["tempo_estimate_missing_or_uncertain"],
    },
    {
      kind: "local_fact",
      name: "beat_grid",
      trackId,
      value: v2?.beatGrid?.beats?.length ?? null,
      confidence: tempoConfidence,
      source: "local_audio_analysis",
      warnings: v2?.beatGrid?.warnings || [],
    },
    {
      kind: "local_fact",
      name: "onset_density",
      trackId,
      value: v2?.onsets?.densityPerMinute
        ? round(v2.onsets.densityPerMinute, 1)
        : round(onsetDensityProxy),
      confidence: v2?.onsets?.confidence ?? (curve.length ? 0.42 : 0),
      source: "local_audio_analysis",
      warnings: v2?.onsets?.warnings || [
        "onset_density_is_proxy_from_energy_changes_not_true_onset_detection",
      ],
    },
    {
      kind: "local_fact",
      ...
      trackId,
      value: v2?.spectral?.centroidHz?.average
        ? round(v2.spectral.centroidHz.average, 1)
        : null,
      confidence: v2?.spectral?.centroidHz?.confidence ?? 0,
      source: "local_audio_analysis",
      warnings: v2 ? [] : ["spectral_centroid_unavailable"],
    },
    {
      kind: "local_fact",
      name: "spectral_rolloff",
      trackId,
      value: v2?.spectral?.rolloffHz?.average
        ? round(v2.spectral.rolloffHz.average, 1)
        : null,
      confidence: v2?.spectral?.rolloffHz?.confidence ?? 0,
      source: "local_audio_analysis",
      warnings: v2 ? [] : ["spectral_rolloff_unavailable"],
    },
    {
      kind: "local_fact",
      name: "spectral_flux",
      trackId,
      value: v2?.spectral?.flux?.average
        ? round(v2.spectral.flux.average)
        : null,
      confidence: v2?.spectral?.flux?.confidence ?? 0,
      source: "local_audio_analysis",
      warnings: v2 ? [] : ["spectral_flux_unavailable"],
    },
    {
      kind: "local_fact",
      name: "key_estimate",
      trackId,
      value: v2?.tonal?.keyEstimate ?? null,
      confidence: v2?.tonal?.confidence ?? 0,
      source: "local_audio_analysis",
      warnings: v2?.tonal?.warnings || ["key_not_detected_locally"],
    },
    {
      kind: "local_fact",
      name: "brightness_proxy",
      trackId,
      value: round(brightnessProxy),
      confidence:
        v2?.spectral?.brightness?.confidence ?? (curve.length ? 0.45 : 0),
      source: "local_audio_analysis",
      warnings: v2
        ? ["brightness_from_local_spectral_descriptors"]
        : ["brightness_is_proxy_not_true_spectral_centroid"],
    },
    {
      kind: "local_fact",
      name: "sonic_density_proxy",
      trackId,
      value: round(sonicDensityProxy),
      confidence: v2?.spectral?.density?.confidence ?? (curve.length ? 0.5 : 0),
      source: "local_audio_analysis",
      warnings: v2
        ? ["density_from_onset_flux_rms_descriptors"]
        : ["density_is_proxy_from_energy_distribution"],
    },
    {
      kind: "local_fact",
      name: "dynamic_range_proxy",
      trackId,
      value: round(dynamicRangeProxy),
      confidence: curve.length ? 0.55 : 0,
      source: "local_audio_analysis",
      warnings: ["dynamic_range_is_proxy"],
    },
  ];
  ...
  for (const silence of analysis.silence || []) {
    if (silence.start !== undefined && silence.end !== undefined) {
      localFacts.push({
        ...
        value: round(
          silence.duration || Math.max(0, silence.end - silence.start),
          1,
        ),
        confidence: 0.8,
        source: "local_audio_analysis",
  ...
  if (duration) {
    rawSections.push(
      makeSection(
        ...
        sectionIndex++,
        0,
        Math.min(duration, Math.max(12, duration * 0.12)),
        [
          "intro_candidate",
          avgEnergy < 0.45 ? "transition_safe_zone" : "stable_groove_section",
        ],
        0.65,
        ["duration", "energy_curve"],
      ),
    );
    rawSections.push(
      makeSection(
        trackId,
        sectionIndex++,
        Math.max(0, duration - Math.max(12, duration * 0.12)),
        duration,
        ["outro_candidate", "transition_safe_zone"],
        0.65,
        ["duration", "energy_curve"],
      ),
    );
  ...
  for (const segment of v2?.segments || []) {
    ...
    if (segment.labels.includes("intro_candidate"))
      labels.push("intro_candidate", "transition_safe_zone");
    if (segment.labels.includes("clean_exit_candidate"))
      labels.push("outro_candidate", "transition_safe_zone");
    if (segment.labels.includes("chorus_like_candidate"))
      labels.push("likely_hook_candidate", "high_energy_section");
    if (segment.labels.includes("first_strong_entrance"))
      labels.push("dynamic_lift_section");
    if (segment.labels.includes("breakdown_or_reset_candidate"))
      labels.push(
        "low_energy_reset_section",
        "breakdown_section",
        "transition_safe_zone",
      );
    if (segment.labels.includes("pre_finale_build_candidate"))
      labels.push("dynamic_lift_section");
    if (segment.labels.includes("finale_candidate"))
      labels.push("finale_candidate", "high_energy_section");
  ...
  for (const candidate of analysis.candidateSections || []) {
    ...
    const end = Math.min(
      duration || candidate.end,
      Math.max(candidate.end, start + minUsefulSec),
    );
    ...
    if (energy >= 0.86 || start > duration * 0.6)
    ...
    rawSections.push(
      makeSection(
        ...
        labels,
        0.6 + energy * 0.25,
        ["rms_energy", "energy_curve", "onset_density"],
        ["hook_is_heuristic", "no_lyrics_available"],
      ),
    );
  }
  ...
  for (const silence of analysis.silence || []) {
    if (
      silence.start === undefined ||
      silence.end === undefined ||
      (silence.duration || 0) < 0.4
    )
      continue;
  ...
  const sectionScores = sections.map((section) => {
    ...
    const boundaryBonus = section.labels.includes("transition_safe_zone")
      ? 0.2
      : 0;
    const beatBonus = section.labels.includes("beat_aligned_candidate")
      ? 0.14
      : 0;
    ...
    const hookStrength = clamp01(
      p * 0.42 +
        e * 0.28 +
        (section.labels.includes("likely_hook_candidate") ? 0.14 : 0) +
        sonicDensityProxy * 0.08 +
        beatBonus * 0.08,
    );
    const entryQuality = clamp01(
      (1 - Math.abs(e - 0.58)) * 0.38 +
        introBonus +
        hookStrength * 0.22 +
        boundaryBonus +
        beatBonus,
    ...
    const exitQuality = clamp01(
      (1 - e) * 0.22 +
        ...
        boundaryBonus +
        beatBonus +
        (section.labels.includes("low_energy_reset_section") ? 0.22 : 0) +
        section.confidence * 0.15,
    );
    const resetUsefulness = clamp01(
      (section.labels.includes("low_energy_reset_section") ? 0.65 : 0) +
        (1 - e) * 0.25 +
        boundaryBonus,
    );
    const finalePotential = clamp01(
      hookStrength * 0.5 +
        p * 0.25 +
        (section.labels.includes("finale_candidate") ? 0.2 : 0) +
        (section.startSec > duration * 0.55 ? 0.1 : 0),
    );
    const beatAlignment = clamp01(
      (section.labels.includes("beat_aligned_candidate") ? 0.82 : 0.38) +
        tempoConfidence * 0.18,
    );
    const downbeatConfidence = clamp01(
      (section.labels.includes("beat_aligned_candidate") ? 0.55 : 0.22) +
        tempoConfidence * 0.35,
    );
    const harmonicStability = clamp01(
      (v2?.tonal?.confidence ?? 0) * 0.8 + (v2?.tonal?.keyEstimate ? 0.15 : 0),
    );
    const spectralContrast = clamp01(
      Math.abs(
        (v2?.spectral?.brightness?.average ?? brightnessProxy) - avgEnergy,
      ) *
        0.55 +
        (v2?.spectral?.flux?.average ?? 0) * 1.8,
    );
    const grooveContinuity = clamp01(
      tempoConfidence * 0.42 +
        beatAlignment * 0.36 +
        (1 - Math.abs(e - avgEnergy)) * 0.22,
    );
    const transitionShockRisk = clamp01(
      (1 - beatAlignment) * 0.3 +
        Math.abs(e - avgEnergy) * 0.3 +
        (1 - section.confidence) * 0.25 +
        (v2?.quality?.clippingRisk ?? 0) * 0.15,
    );
    const scores = {
      hookStrength: round(hookStrength),
      entryQuality: round(entryQuality),
      exitQuality: round(exitQuality),
      transitionUsability: round(
        clamp01((entryQuality + exitQuality + resetUsefulness) / 3),
      ),
      ...
      buildUsefulness: round(
        clamp01(Math.max(0, e - avgEnergy) + dynamicRangeProxy * 0.25),
      ),
    ...
    const scoreWarnings = Array.from(
      new Set(
        [
          ...section.warnings,
  ...
  const heuristicGuesses: HeuristicGuess[] = sectionScores.flatMap((score) => {
    ...
    if (score.scores.hookStrength >= 0.58)
      guesses.push({
        kind: "heuristic_guess",
        name: "likely_hook_candidate",
        trackId,
        sectionId: score.sectionId,
        score: score.scores.hookStrength,
        confidence: score.confidence,
        reason:
          "High energy/peak section; not confirmed as chorus or true hook.",
        factsUsed: score.factsUsed,
        warnings: ["hook_is_heuristic", "no_lyrics_available"],
      });
    if (score.scores.entryQuality >= 0.58)
      guesses.push({
        kind: "heuristic_guess",
        name: "strong_entry_candidate",
        trackId,
        sectionId: score.sectionId,
        score: score.scores.entryQuality,
        confidence: score.confidence,
        reason: "Good entry proxy from boundary quality and usable energy.",
        factsUsed: score.factsUsed,
        warnings: ["entry_quality_is_heuristic"],
      });
    if (score.scores.exitQuality >= 0.58)
      guesses.push({
        kind: "heuristic_guess",
        name: "strong_exit_candidate",
        trackId,
        sectionId: score.sectionId,
        score: score.scores.exitQuality,
        confidence: score.confidence,
        reason: "Good exit proxy from outro/reset/low energy boundary.",
        factsUsed: score.factsUsed,
        warnings: ["exit_quality_is_heuristic"],
      });
    if (score.scores.resetUsefulness >= 0.55)
      guesses.push({
        kind: "heuristic_guess",
        name: "low_energy_reset_zone",
        trackId,
        sectionId: score.sectionId,
        score: score.scores.resetUsefulness,
        confidence: score.confidence,
        reason: "Low energy or silence-adjacent section can reset the medley.",
        factsUsed: score.factsUsed,
        warnings: [],
      });
    if (score.scores.finalePotential >= 0.62)
      guesses.push({
        kind: "heuristic_guess",
        name: "finale_candidate",
        trackId,
        sectionId: score.sectionId,
        score: score.scores.finalePotential,
        confidence: score.confidence,
        reason: "High energy or late-track section may work as a finale.",
        factsUsed: score.factsUsed,
        warnings: ["finale_role_is_heuristic"],
      });
    return guesses;
  });

  const sortBy = (key: keyof SectionScore["scores"]) =>
    [...sectionScores]
...
function bestSection(
  ...
  key: keyof SectionScore["scores"],
) {
  return (
    [...track.sectionScores].sort((a, b) => b.scores[key] - a.scores[key])[0] ||
    track.sectionScores[0]
  );
}

// Simple historical success scorer from permanent wisdom
function historicalSuccessBonus(
  fromTrackId: string,
  toTrackId: string,
  wisdom: any[],
): number {
  if (!wisdom.length) return 0;
  const relevant = wisdom.filter(
    (w) =>
      w.type === "completed_medley" &&
      w.designPlan?.transitions?.some(
        (t: any) => t.fromTrackId === fromTrackId && t.toTrackId === toTrackId,
      ),
  );
  ...
  const scores = relevant
    .map((w) => w.metrics?.overallScore || 70)
    .filter(Boolean);
...
function transitionTypeFromScores(
  scores: TransitionScore["scores"],
): TransitionScore["transitionType"] {
...
function computeSectionPairTransition(
  ...
  const fromSection = fromTrack.sections.find(
    (s) => s.sectionId === fromScore.sectionId,
  );
  const toSection = toTrack.sections.find(
    (s) => s.sectionId === toScore.sectionId,
  );
  ...
  const tempo = scoreTempoCompatibility(
    fromTrack.profile.tempoEstimate,
    toTrack.profile.tempoEstimate,
  );
  const harmonic = scoreHarmonicCompatibility(
    fromTrack.profile,
    toTrack.profile,
  );
  const brightnessCompatibility = clamp01(
    1 -
      Math.abs(
        fromTrack.profile.brightnessProxy - toTrack.profile.brightnessProxy,
      ),
  );
  const densityCompatibility = clamp01(
    1 -
      Math.abs(
        fromTrack.profile.sonicDensityProxy - toTrack.profile.sonicDensityProxy,
      ),
  );
  const beatAlignment = clamp01(
    ((fromScore.scores.beatAlignment ?? 0.35) +
      (toScore.scores.beatAlignment ?? 0.35) +
      fromTrack.profile.tempoConfidence +
      toTrack.profile.tempoConfidence) /
      4,
  );
  const grooveContinuity = clamp01(
    tempo.score * 0.32 +
      beatAlignment * 0.28 +
      energyContinuity * 0.2 +
      densityCompatibility * 0.2,
  );
  const spectralContrast = clamp01(
    Math.abs(
      fromTrack.profile.brightnessProxy - toTrack.profile.brightnessProxy,
  ...
  const sectionBoundaryQuality = clamp01(
    (fromScore.scores.exitQuality + toScore.scores.entryQuality) / 2,
  );

  const smoothBlend = clamp01(
    energyContinuity * 0.16 +
      tempo.score * 0.2 +
      harmonic.score * 0.2 +
      beatAlignment * 0.18 +
      brightnessCompatibility * 0.12 +
      densityCompatibility * 0.06 +
      sectionBoundaryQuality * 0.08,
  );
  const hardCut = clamp01(
    sectionBoundaryQuality * 0.36 +
      energyContrast * 0.22 +
      beatAlignment * 0.14 +
      toScore.scores.entryQuality * 0.18 +
      (fromScore.scores.exitQuality > 0.65 ? 0.1 : 0),
  );
  const energyLift = clamp01(
    Math.max(0, energyDiff) * 0.55 +
      toScore.scores.buildUsefulness * 0.25 +
      toScore.scores.entryQuality * 0.2,
  );
  const energyDrop = clamp01(
    Math.max(0, -energyDiff) * 0.55 +
      toScore.scores.resetUsefulness * 0.3 +
      sectionBoundaryQuality * 0.15,
  );
  const resetMoment = clamp01(
    fromScore.scores.resetUsefulness * 0.45 +
      toScore.scores.entryQuality * 0.3 +
      energyContrast * 0.15 +
      sectionBoundaryQuality * 0.1,
  );
  const buildTransition = clamp01(
    energyLift * 0.55 + toScore.scores.hookStrength * 0.25 + tempo.score * 0.2,
  );
  const finaleLaunch = clamp01(
    toScore.scores.finalePotential * 0.55 + energyLift * 0.25 + hardCut * 0.2,
  );
  const surpriseContrast = clamp01(
    energyContrast * 0.28 +
      spectralContrast * 0.24 +
      hardCut * 0.24 +
      toScore.scores.hookStrength * 0.18 +
      (1 - harmonic.score) * 0.06,
  );
  const transitionShockRisk = clamp01(
    (1 - beatAlignment) * 0.24 +
      (1 - harmonic.score) * 0.18 +
      energyContrast * 0.24 +
      (fromScore.scores.transitionShockRisk ?? 0.3) * 0.14 +
      (toScore.scores.transitionShockRisk ?? 0.3) * 0.14 +
      (tempo.warning || harmonic.warning ? 0.06 : 0),
  );
  const riskLevel = clamp01(
    (1 - tempo.score) * 0.18 +
      (1 - harmonic.score) * 0.16 +
      (1 - sectionBoundaryQuality) * 0.27 +
      energyContrast * 0.18 +
      transitionShockRisk * 0.18 +
      (tempo.warning ? 0.03 : 0),
  );
  ...
  return {
    ...
    warnings: [
      tempo.warning,
      harmonic.warning,
      riskLevel > 0.6 ? "transition_risk_high" : "",
      "section_pair_heuristic",
    ].filter(Boolean),
  };
}
    ...
    for (const to of tracks) {
      ...
      for (const fromScore of exitCands) {
        for (const toScore of entryCands) {
          const local = scoreSectionPairLocal(
            from,
            to,
            fromScore.sectionId,
            toScore.sectionId,
      ...
      for (const cand of topCandidates) {
        ...
        const pairData = computeSectionPairTransition(
          from,
          to,
          cand.fromScore,
          cand.toScore,
...
function greedyOrder(
  ...
  seed: TrackIntelligence,
  strategy: "smooth" | "lift" | "contrast",
) {
  ...
  while (remaining.size) {
    ...
    const candidates = matrix.filter(
      (item) => item.fromTrackId === current && remaining.has(item.toTrackId),
    );
    const sorted = candidates.sort((a, b) => {
      const aScore =
        strategy === "smooth"
          ? a.scores.smoothBlend
          : strategy === "lift"
            ? a.scores.energyLift + a.scores.buildTransition
            : a.scores.surpriseContrast;
      const bScore =
        strategy === "smooth"
          ? b.scores.smoothBlend
          : strategy === "lift"
            ? b.scores.energyLift + b.scores.buildTransition
            : b.scores.surpriseContrast;
...
function buildStrategy(
  ...
  const orderedTracks = orderedIds.map((trackId, index) => {
    ...
    const bestInbound = prevId
      ? matrix
          .filter((m) => m.fromTrackId === prevId && m.toTrackId === trackId)
          .sort((a, b) => b.score - a.score)[0]
      : undefined;

    const bestOutbound = nextId
      ? matrix
          .filter((m) => m.fromTrackId === trackId && m.toTrackId === nextId)
          .sort((a, b) => b.score - a.score)[0]
      : undefined;

    const entrySec =
      bestInbound?.toEntrySec ??
      track.sections.find(
        (s) =>
          s.sectionId ===
          (bestSection(track, "entryQuality") || bestSection(track, sectionKey))
            ?.sectionId,
      )?.startSec ??
      0;

    const exitSec =
      bestOutbound?.fromExitSec ??
      track.sections.find(
        (s) =>
          s.sectionId ===
          (bestSection(track, "exitQuality") || bestSection(track, sectionKey))
            ?.sectionId,
      )?.endSec ??
    ...
    const selectedSectionIds = Array.from(
      new Set(
        [bestInbound?.toSectionId, bestOutbound?.fromSectionId].filter(Boolean),
      ),
    ) as string[];

    if (!selectedSectionIds.length) {
      const fallback =
        bestSection(track, sectionKey) || bestSection(track, "hookStrength");
  ...
  const transitions = orderedIds
    .slice(0, -1)
    .map((trackId, index) => {
      const nextId = orderedIds[index + 1];
      return matrix
        .filter(
          (item) => item.fromTrackId === trackId && item.toTrackId === nextId,
        )
        .sort((a, b) => b.score - a.score)[0];
    })
  ...
  const strategyScore =
    average([
      ...
      ...orderedTracks.map((track) => {
        ...
        const section = source?.sectionScores.find(
          (item) => item.sectionId === track.selectedSectionIds[0],
        );
    ...
    historicalSuccessBonus(
      ...
      wisdom || [],
    ); // light cross-medley wisdom boost
  ...
  return {
    ...
    confidence: round(
      average([
        ...transitions.map((item) => item.confidence),
        ...tracks.map((track) => track.profile.confidence),
      ]),
    ),
    estimatedDurationSec: round(
      orderedTracks.reduce(
        (sum, item) => sum + Math.max(0, item.exitSec - item.entrySec),
        0,
      ),
    ),
    orderedTracks,
...
export function buildOrderStrategies(
  tracks: TrackIntelligence[],
  matrix: TransitionScore[],
  wisdom: any[] = [],
): MedleyOrderStrategy[] {
  if (!tracks.length) return [];
  const byEnergy = [...tracks].sort(
    (a, b) => a.profile.averageEnergy - b.profile.averageEnergy,
  );
  const byHook = [...tracks].sort(
    (a, b) =>
      (bestSection(b, "hookStrength")?.scores.hookStrength ?? 0) -
      (bestSection(a, "hookStrength")?.scores.hookStrength ?? 0),
  );
  ...
  return [
    buildStrategy(
      ...
      matrix,
      "transitionUsability",
      ["May sacrifice peak drama for blend quality."],
      ["Key is not locally confirmed."],
      wisdom,
    ),
    buildStrategy(
      "best_emotional_arc_proxy",
      "Best Emotional Arc Proxy",
      liftOrder,
      tracks,
      matrix,
      "buildUsefulness",
      [
        "Emotional role is inferred from energy, brightness, density, and dynamics only.",
      ],
      ["Not a lyric or meaning analysis."],
      wisdom,
    ),
    buildStrategy(
      "highest_intensity",
      ...
      matrix,
      "hookStrength",
      ["Can feel relentless if every section is loud."],
      ["Intensity is based on local energy proxies."],
      wisdom,
    ),
    buildStrategy(
      "surprise_contrast",
      "Most Surprising Creative Contrast",
      contrastOrder,
      tracks,
      matrix,
      "contrastValue",
      ["More creative risk and less guaranteed smoothness."],
      ["Surprise is heuristic."],
      wisdom,
    ),
    buildStrategy(
      "live_showcase",
      "Best Live Performance Showcase",
      liftOrder,
      tracks,
      matrix,
      "finalePotential",
      ["Optimizes performance arc over technical smoothness."],
      [
        "Vocal intensity is not confirmed unless cloud listening or user notes are provided.",
      ],
      wisdom,
    ),
    buildStrategy(
      "shortest_strong_medley",
      "Shortest Strong Medley",
      shortestOrder,
      tracks,
      matrix,
      "hookStrength",
      ["Leaves out lower-ranked material."],
      ["Short strategy prioritizes hook proxies."],
      wisdom,
    ),
...
export function buildMedleyDesignPayload(input: {
  ...
  const strategies = buildOrderStrategies(
    input.tracks,
    transitionMatrix,
    input.wisdom || [],
  );
  ...
  const allSectionsWithScores = input.tracks.flatMap(
    (track) => track.sectionScores,
  );
  ...
  const introCandidates = allSectionsWithScores
    .map((score) => {
      const track = input.tracks.find(
        (t) => t.profile.trackId === score.trackId,
      )!;
      const section = track.sections.find(
        (s) => s.sectionId === score.sectionId,
      );
      const isExplicitIntro =
        section?.labels.some((l) =>
          [
            "intro_candidate",
            "first_strong_entrance",
            "clean_exit_candidate",
          ].includes(l as string),
        ) ?? false;
      const entryBonus = score.scores.entryQuality * 0.45;
      const lowToMidEnergy =
        (1 - Math.min(score.scores.energyRole, 0.85)) * 0.25; // prefer not starting at 100% intensity
  ...
  const finaleCandidates = allSectionsWithScores
    .map((score) => {
      ...
      const isExplicitFinale =
        section?.labels.some((l) =>
          ["finale_candidate", "pre_finale_build_candidate"].includes(
            l as string,
          ),
        ) ?? false;
  ...
  return {
    ...
    transitionMatrixSummary: transitionMatrix.slice(
      0,
      input.maxTransitions ?? 24,
    ),
...
export function validateAiMedleyPlan(plan: any, payload: MedleyDesignPayload) {
  ...
  if (
    /\blyrics?\b/.test(serialized) &&
    !serialized.includes("user-provided") &&
    !serialized.includes("unavailable")
  ) {
    errors.push(
      "AI response appears to claim lyrics without a provided lyric source.",
    );
  }
  if (
    /\bkey\b/.test(serialized) &&
    !serialized.includes("unknown") &&
    !serialized.includes("not confirmed")
  ) {
    warnings.push(
      "AI response mentions key; verify it is not presented as a local fact.",
    );
  }

  const referencedSectionIds =
    serialized.match(/[a-z0-9_-]+_section_\d+/gi) || [];
  for (const id of referencedSectionIds) {
    if (!sectionIds.has(id))
      errors.push(`AI response referenced unsupported section id: ${id}`);
  }
...
export function evaluateSectionPair(
  ...
  const fromScore = fromTrack.sectionScores.find(
    (s) => s.sectionId === fromSectionId,
  );
  const toScore = toTrack.sectionScores.find(
    (s) => s.sectionId === toSectionId,
  );
...
export function scoreSectionPairLocal(
  ...
  const beatAlign = clamp01(
    ((fromScore.scores.beatAlignment ?? 0.4) +
      (toScore.scores.beatAlignment ?? 0.4)) /
      2 +
  ...
  const brightnessCompat = clamp01(
    1 -
      Math.abs(
        fromTrack.profile.brightnessProxy - toTrack.profile.brightnessProxy,
      ),
  );
  const densityCompat = clamp01(
    1 -
      Math.abs(
        fromTrack.profile.sonicDensityProxy - toTrack.profile.sonicDensityProxy,
      ),
  );
  components.spectralCompatibility = clamp01(
    (brightnessCompat + densityCompat) / 2,
  );

  // Intro / outro suitability (small bonus if from is good exit and to is good entry)
  const boundaryQuality = clamp01(
    (fromScore.scores.exitQuality + toScore.scores.entryQuality) / 2,
  );
  ...
  if (components.tempo <= 0.55 || components.harmonic <= 0.5) {
    score = score * 0.68;
  }

  // Extra penalty for very poor tempo or harmonic match
  if (components.tempo < 0.4 || components.harmonic < 0.4) {
    score = score * 0.55;
  }

  const confidence = clamp01(
    (fromTrack.profile.confidence +
      toTrack.profile.confidence +
      tempo.confidence +
      harmonic.confidence) /
      4,
  );
  ...
  return {
    ...
    components: Object.fromEntries(
      Object.entries(components).map(([k, v]) => [k, round(v)]),
    ),
    reason,
  };
}
