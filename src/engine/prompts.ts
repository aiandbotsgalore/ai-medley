...
const TOOL_DEFINITIONS = [
  {
    ...
    description: 'Execute a shell command in the session work directory. Use ONLY for lightweight filesystem diagnostics (dir, ls, cat, find, echo). Do NOT run ffmpeg commands here. Do NOT use this for rendering, concatenating, or exporting audio. The final MP3 must be produced by calling finalize_medley — not by this tool.',
    parameters: {
      type: 'object',
  ...
  },
  {
    ...
    description: 'Report current quality scores. ALL scores MUST be integers on a 0–100 scale (e.g. 85 means 85% quality, not 0.85). DO NOT use decimals or 0–1 values. The score is NOT a permission gate — any score value does NOT authorize or require manual FFmpeg work. Call this once after transitions are applied, then immediately call finalize_medley regardless of score. Do not attempt execute_shell_command after this.',
    parameters: {
      type: 'object',
      properties: {
        emotionalArc: { type: 'number', description: 'Emotional arc quality. Integer 0–100. Example: 82' },
        transitionSmoothness: { type: 'number', description: 'Transition smoothness quality. Integer 0–100. Example: 79' },
        performerIdentity: { type: 'number', description: "How well each performer's identity is preserved. Integer 0–100. Example: 85" },
        overallScore: { type: 'number', description: 'Overall medley quality. Integer 0–100. Example: 83' },
        iteration: { type: 'number', description: 'Current iteration number' },
        phase: { type: 'string', description: 'Current phase: ANALYZE | DESIGN | BUILD | FINISH' }
      },
  ...
  {
    name: 'finish_medley',
    description: 'DEPRECATED — does NOT render an MP3. Returns acknowledged only. Do NOT call this if you want to produce the final audio file. Use finalize_medley instead.',
    parameters: {
      type: 'object',
      properties: {
