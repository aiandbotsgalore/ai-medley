    ...
    <header className="min-h-16 border-b border-[#1E1E1E] flex items-center justify-between gap-2 px-3 sm:px-6 py-2 bg-[#0C0C0C] shrink-0 relative overflow-hidden">
      ...
      <div className="flex items-center gap-2 sm:gap-4 min-w-0 relative z-10">
        ...
        <div className="min-w-0">
          <h1 className="text-[13px] sm:text-[15px] font-bold tracking-tight text-white leading-tight">AI Medley Architect</h1>
          <span className="hidden sm:block text-[10px] font-mono text-[#555] tracking-wider">v3.1.0 — {provider === 'gemini' ? 'GEMINI DIRECT' : 'OPENROUTER'}</span>
        </div>
      </div>
      <div className="flex items-center gap-2 sm:gap-5 shrink-0 relative z-10">
        ...
        {currentModel && (
          ...
          >
            ...
            {currentRole && <span className="text-amber-400 uppercase">{currentRole}</span>}
            <span className="text-[#00F0FF] max-w-[260px] truncate">{currentModel}</span>
          </div>
        )}

        <div className="flex items-center gap-2.5 px-2 sm:px-3 py-1.5 rounded-full bg-[#111] border border-[#222]">
          ...
          <span className={`hidden sm:inline text-[10px] font-mono uppercase tracking-widest transition-colors ${isActive ? 'text-[#00F0FF]' : 'text-[#666]'}`}>
