...
import type {
  ...
  SectionScore,
  TransitionScore,
} from "../engine/medleyIntelligence";
...
function ScorePill({ label, value }: { label: string; value: number }) {
  return (
    <span
      className={`text-[9px] font-mono px-2 py-1 rounded border border-[#222] bg-[#0A0A0A] ${scoreColor(value)}`}
    >
      {label}: {pct(value)}
    </span>
  );
  ...
  return (
    <div className="border border-[#1E1E1E] bg-[#0D0D0D] rounded-md p-2">
      <div className="flex items-center justify-between gap-2">
        <div className="text-[10px] font-mono text-[#AAA] truncate">
          {score.sectionId}
        </div>
      ...
      {score.warnings.length > 0 && (
        <div
          className="mt-2 text-[9px] text-amber-300/80 truncate"
          title={score.warnings.join(", ")}
        >
...
const TopTransition: React.FC<{ transition: TransitionScore }> = ({
  transition,
}) => {
  return (
    <div className="border border-[#1E1E1E] bg-[#0D0D0D] rounded-md p-2">
      ...
      <div className="mt-1 text-[9px] text-[#666]">
        ...
        {transition.fromExitSec.toFixed(1)}s →{" "}
        {transition.toEntrySec.toFixed(1)}s
      </div>
      <div className="mt-2 flex flex-wrap gap-1.5">
        ...
        <ScorePill
          label="Key"
          value={transition.scores.harmonicCompatibility ?? 0}
        />
      </div>
    </div>
  );
...
export default function MedleyMatchPanel({
  design,
}: {
  ...
  const advancedTracks = design.tracks.filter(
    (track) => track.advancedAnalysisAvailable,
  );
    ...
    <aside className="w-full md:w-96 min-h-[420px] md:min-h-0 border-t md:border-t-0 md:border-l border-[#1A1A1A] bg-[#090909] flex flex-col shrink-0">
      <div className="p-5 border-b border-[#1A1A1A]">
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
          ...
          <div className="border border-[#222] bg-[#0A0A0A] rounded-md p-2">
            ...
            <div className="text-[#00F0FF] text-lg">
              {design.transitionMatrixSummary.length}
            </div>
          </div>
          <div className="border border-[#222] bg-[#0A0A0A] rounded-md p-2">
            <div className="text-[#555]">Advanced</div>
            <div className="text-[#00F0FF] text-lg">
              {advancedTracks.length}
            </div>
          </div>
          <div className="border border-[#222] bg-[#0A0A0A] rounded-md p-2">
            <div className="text-[#555]">Key Est.</div>
            <div className="text-[#00F0FF] text-lg">
              {design.tracks.filter((track) => track.keyEstimate).length}
            </div>
          </div>
        </div>
        <div className="mt-3 text-[10px] leading-relaxed text-[#777]">
          Facts, heuristic guesses, scoring, and AI reasoning are separated.
          Hook and emotion labels are proxies unless user notes or allowed clip
          listening confirm them.
        </div>
      </div>
      ...
      <div className="flex-1 p-5 overflow-y-auto custom-scrollbar space-y-5">
        <section>
          ...
          <div className="space-y-2">
            {design.tracks.slice(0, 5).map((track) => (
              <div
                key={track.trackId}
                className="border border-[#1E1E1E] bg-[#0D0D0D] rounded-md p-2"
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[10px] font-mono text-[#AAA] truncate">
                    {track.filename}
                  ...
                  <span className="text-[9px] text-[#555]">
                    conf {pct(track.confidence)}
                  </span>
                </div>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  <ScorePill
                    label="Beat"
                    value={track.beatConfidence ?? track.tempoConfidence}
                  />
                ...
                <div className="mt-2 text-[9px] text-[#666]">
                  BPM {track.tempoEstimate ?? "unknown"} • Key{" "}
                  {track.keyEstimate ?? "unknown"} • Onsets{" "}
                  {track.onsetDensityPerMinute
                    ? `${track.onsetDensityPerMinute.toFixed(1)}/min`
                    : "unknown"}
                </div>
              </div>
            ))}
        ...
        <section>
          ...
          <div className="space-y-2">
            {topSections.map((score) => (
              <TopSection key={score.sectionId} score={score} />
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-2">
            {topStrategies.map((strategy) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[10px] font-bold text-[#CCC]">
                    {strategy.title}
                  </div>
                  <span
                    className={`text-[9px] font-mono ${scoreColor(strategy.score)}`}
                  >
                    {pct(strategy.score)}
                  </span>
                </div>
                <div className="mt-1 text-[9px] text-[#666]">
                  {strategy.orderedTracks
                    .map((track) => track.trackId)
                    .join(" → ")}
                </div>
                {strategy.tradeoffs.length > 0 && (
                  <div className="mt-2 text-[9px] text-[#888]">
                    {strategy.tradeoffs[0]}
                  </div>
                )}
              </div>
            ))}
        ...
        {design.warnings.length > 0 && (
          <section className="border border-amber-500/20 bg-amber-500/5 rounded-md p-3">
            ...
            <ul className="space-y-1 text-[9px] text-amber-100/70">
              {design.warnings.slice(0, 5).map((warning) => (
