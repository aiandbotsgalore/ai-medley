...
for (let i = 0; i < transitions.length; i++) {
  ...
  if (i === 0) {
    segments.push({
      srcPath: trackPathMap[t.fromTrackId],
      start: 0,
      end: fromExit,
      label: "seg_0",
    });
  ...
  segments.push({
    srcPath: trackPathMap[t.toTrackId],
    start: toEntry,
    end: null,
...
for (let i = 0; i < xfadeDurations.length; i++) {
  ...
  if (style === "mashup_layer") {
    ...
    filterLines.push(
      `[${nextLabel}]equalizer=f=250:width_type=h:width=2:g=-4,equalizer=f=4000:width_type=h:width=2:g=-3[${layerLabel}]`,
    );
    filterLines.push(
      `[${currentLabel}][${layerLabel}]amix=inputs=2:duration=first:dropout_transition=0[${mixedLabel}]`,
    );
    ...
    filterLines.push(
      `[${currentLabel}][${nextLabel}]acrossfade=d=${d}:curve1=tri:curve2=tri[${xfade}]`,
    );
    currentLabel = xfade;
  }
}
...
execFile(
  ffmpegPath!,
  finalArgs,
  { cwd: workDir, timeout: 180000, windowsHide: true },
  (err, stdout, stderr) => {
    ...
    const hasErrorJson = fs.existsSync(
      path.join(workDir, "finalize_error.json"),
    );
    const hasAlimiterError =
      combinedStderr.includes("alimiter") &&
      combinedStderr.toLowerCase().includes("out of range");
    const mp3Exists = fs.existsSync(outputPath);
    const mp3Size = mp3Exists ? fs.statSync(outputPath).size : 0;
    const hasBranchingInGraph =
      fullGraph.includes("amix") && fullGraph.includes("mash_");
    const mashupBranchesReported = mashupBranches > 0;

    console.log(
      `1. No finalize_error.json:          ${!hasErrorJson ? "PASS" : "FAIL"}`,
    );
    console.log(
      `2. No alimiter error in stderr:     ${!hasAlimiterError ? "PASS" : "FAIL"}`,
    );
    console.log(
      `3. Output from pure-clean path:     ${mp3Exists && mp3Size > 100000 ? "PASS" : "FAIL"} (${(mp3Size / 1024 / 1024).toFixed(2)} MB)`,
    );
    console.log(
      `4. mashupBranches > 0:              ${mashupBranchesReported ? "PASS" : "FAIL"} (detected ${mashupBranches})`,
    );
    console.log(
      `5. Branching visible in graph:      ${hasBranchingInGraph ? "PASS" : "FAIL"}`,
    );
    ...
    if (allGood) {
      ...
      const mashupLines = graphLines.filter(
        (l) =>
          l.includes("mash_") ||
          l.includes("layer_") ||
          l.includes("mixed_") ||
