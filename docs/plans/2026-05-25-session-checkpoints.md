...
app.post("/api/checkpoint", express.json({ limit: "50mb" }), (req, res) => {
  ...
  fs.writeFileSync(
    filePath,
    JSON.stringify({ ...req.body, savedAt: new Date().toISOString() }),
  );
  res.json({ ok: true });
});
...
app.get("/api/checkpoints", (_req, res) => {
  ...
  const checkpoints = fs
    ...
    .map((f) => {
      try {
        return JSON.parse(
          fs.readFileSync(path.join(CHECKPOINT_DIR, f), "utf8"),
        );
      } catch {
        return null;
      }
...
Add this function _inside_ `runAutonomousLoop`, right after `session` is first created (after `let session = createSessionForModel(getCurrentModel());`):
...
Note: `saveCheckpoint` closes over `session`, `iterations`, `currentModelIndex`, etc. Since these are `let` variables in the outer function scope, the closure captures the _binding_, not the _value_ — so calling `saveCheckpoint()` at any point reads the current values. This is correct.
...
let session = createSessionForModel(
  getCurrentModel(),
...
if (resumeState?.sectionPairCacheEntries) {
  for (const [k, v] of resumeState.sectionPairCacheEntries)
    sectionPairCache.set(k, v);
}

if (resumeState?.evaluationsPerFromSectionEntries) {
  for (const [k, v] of resumeState.evaluationsPerFromSectionEntries)
    evaluationsPerFromSection.set(k, v);
}
```
